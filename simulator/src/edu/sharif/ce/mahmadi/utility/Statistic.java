...
public class Statistic {
    ...
    public void printStatistics(String statFilePath, String statCdfFilePath) {
        ...
        try (BufferedWriter fw = new BufferedWriter(new FileWriter(statFilePath)); BufferedWriter fwCdf = new BufferedWriter(new FileWriter(statCdfFilePath))) {
            ...
            out.setLength(0);
            for (Float i : values) {
                double prob = nrElementsSmallerEqual(values, i) / (double) values.size();
    ...
    public double getRealSTD() {
        double mean = getMean();
        double temp = 0;
        for (double num : values)
            temp += (mean - num) * (mean - num);

        double variance =  nrOfValues < 2 ? 0 : temp / (nrOfValues - 1);
        return Math.sqrt(variance);
    }
    ...
    public double getError() {
        ...
        for (double num : values)
            temp += (mean - num) * (mean - num);
        return values.size() == 0 ? 0 : ((double) Math.sqrt(temp / values.size()) / 2);
    }

    public double getConfidenceInterval() {
        // value for 95% confidence interval, source:
        // https://en.wikipedia.org/wiki/Confidence_interval#Basic_Steps
        double confidenceLevel = 2.776;
        System.out.println(values);
        double temp = values.size() == 0 ? 0 : confidenceLevel * (getRealSTD() / Math.sqrt(nrOfValues));
        return temp;
    }
    ...
    public double getNormalPecentile(int percentile) {
        ...
        if (values.size() == 0) {
            ...
            return ((double) percentile * values.get(0)) / 100;
        }
        if (!sorted) {
            Collections.sort(values);
            sorted = true;
    ...
    public void addReplication(Statistic st) {
        ...
        if (saveValues && st.hasSavedValues()) values.addAll(st.getValues());
    }

    public void removeOutlier() {
        double Q1 = this.getPercentile(25);
        double Q3 = this.getPercentile(75);
        double diff = Q3 - Q1;
        double start = (double) (Q1 - 1.5 * diff);
        double end = (double) (Q3 + 1.5 * diff);
        Iterator<Float> valIterator = values.iterator();
        while (valIterator.hasNext()) {
            double val = valIterator.next();
    ...
    public void addEndValue(double time) {
        if (startTime != -1) addValue((time - startTime));
        startTime = time;
    }
    ...
    public void addSizeValue(double endTime, double value) {
        ...
        addValue((endTime - stateChangeTime) * value);
    ...
    public double getMean(double totalTime) {
        ...
        if (totalTime != startTime && startTime != -1) addValue(totalTime - startTime);
        return getMean();
    }
