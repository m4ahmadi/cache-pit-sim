...
public class AnalyticMahdiehHypoTwoLRUCache extends AnalyticCache {
    ...
    private double qPrimeIntegral(int fId) {
        double rate = popularityDist.getArrivalRate(fId);
        double cv = CV(fId);
        if (cv < 0.09) return CumG(rate, CTime + R(fId)) - CumG(rate, R(fId));
//        if (cv == 0)   return CumG(rate, CTime + meanR(fId)) - CumG(rate, meanR(fId));
...
//        fit.fitFirstMoment(meanR(fId));
    ...
    private double qIntegral(int fId) {
        double rate = popularityDist.getArrivalRate(fId);
        double cv = CV(fId);
        if (cv < 0.09) return CumF(rate, CTime + R(fId)) - CumF(rate, +R(fId));
