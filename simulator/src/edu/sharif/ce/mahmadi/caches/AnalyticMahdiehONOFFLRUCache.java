...
public class AnalyticMahdiehONOFFLRUCache extends AnalyticCache {
    ...
    public AnalyticMahdiehONOFFLRUCache(int id, double downloadRate, Simulation simulation, Dist mpopularityDist, int mcacheSize, double onMean, int offMean) {
        ...
        System.out.println("[" + "F=" + new DecimalFormat("0.0E0").format(popularityDist.getCatalogSize()) + "]" + "[D=" + new DecimalFormat("0.0E0").format(theoreticalDownTime) + "]" + "[c=" + new DecimalFormat("0.0E0").format(cacheLimit) + "]" + "[rate=" + new DecimalFormat("0.0E0").format(popularityDist.getArrivalRate()) + "]" + "[ON=" + T_ON_ + "]" + "[" + T_OFF_ + "]");
        //z = mz;
    }
    ...
    public void setCTime(double mC) {
        CTime = mC;
    }
