...
public class AnalyticHypoTwoLRUCache extends AnalyticCache {
    ...
    public AnalyticHypoTwoLRUCache(int id, double downloadRate, Simulation simulation, Dist mpopularityDist, int mcacheSize, int mz) {
        super(id, downloadRate, simulation, mpopularityDist);
        virtualCache = new AnalyticHypoLRUCache(id, 0, simulation, mpopularityDist, mcacheSize, mz);
    ...
    private double expectedRTwo(int fId) {
        double rate = popularityDist.getArrivalRate(fId);
        double p =((double)z/(z+1));
        double x1 =z*rate;
        double x2 =(1.0/(double)z) * rate;
    ...
    private double varianceR(int fId){
        double variance =  expectedRTwo(fId) - pow(expectedR(fId), 2);
        assert variance>=0;
    ...
    public double computeAnalyticHitProbability(int fId) {
        ...
        double hitPercent = (MCycle(ZDDhit) - counting_function(rate, expectedR(fId))) / (MCycle(ZDDhit));
    ...
    public double computeAnalyticPitHitProbability(int fId) {
        ...
        double rate = popularityDist.getArrivalRate(fId);
        double ZDDhit = computeAnalyticZDDHitProbability(fId);
        double pitPercent = (counting_function(rate, expectedR(fId))) / MCycle(ZDDhit);
        double pit1 = pitPercent >= 1 ? ZDDhit : ZDDhit * pitPercent;
        //PIT hit second part

        double pit2 = (1 - ZDDhit) * (1 - ZDDhit) * CumF(rate, expectedR(fId));
        return (pit1) + pit2;
    }
    ...
    private double cachedim() {
        ...
        for (int f = 1; f <= popularityDist.getCatalogSize(); f++) {
            ...
            double cumg = CumG(rate);
            double cumf = CumF(rate, CTime);
            double pin = (cumg * q)/(1.0-cumf*(1-q));
            result += pin;
