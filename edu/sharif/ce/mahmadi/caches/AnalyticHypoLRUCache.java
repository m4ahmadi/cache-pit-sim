...
public class AnalyticHypoLRUCache extends AnalyticCache {

    private final int cacheLimit;
    private double CTime;
    private int z;

    public AnalyticHypoLRUCache(int id, double downloadRate, Simulation simulation, Dist mpopularityDist, int cacheSize, int mz) {
    ...
    public double computeAnalyticPitHitProbability(int fId) {
        ...
        double rate = popularityDist.getArrivalRate(fId);
        double ZDDHit= CumF(rate);
        double pit = (counting_function(rate,theoreticalDownTime))/(MCycle(ZDDHit)+1);
       // double pit = ((downloadTime) * rate) / (exp);
        if (pit > 1) pit = (counting_function(rate,theoreticalDownTime)) / (counting_function(rate,theoreticalDownTime) + 1);
    ...
    double counting_function (double lambda, double time)
    {
        ...
        //double firstPart = time/expected;
